...
def main(cfg: DictConfig, **kwargs):
    ...
    print(f"Average GFlops per {len(datalines)} samples for task 1: {GFlops/len(datalines)} {flops_unit}")
    print('Number of parameters: {:<8}'.format(params))
    print(f"Average inference time (cap1, cap2 direction) per {len(datalines)} samples for task 1: {inference_time_cap1_cap2/len(datalines)} seconds")
    print(f"Average inference time (cap1, cap2 direction) per {len(datalines)} samples for task 1: {inference_time_cap2_cap1/len(datalines)} seconds")
